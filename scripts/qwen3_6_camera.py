"""Real-time video understanding with Qwen3.6-VL-27B (Qwen3_5ForConditionalGeneration).
...
This drives the *base* Qwen3.6-27B model directly (no fine-tuning) so you can probe
its out-of-the-box video understanding. It is the bigger sibling of
`scripts/qwen3_5_camera.py`; both share the same `Qwen3_5ForConditionalGeneration`
architecture (`model_type: qwen3_5`), so the load + inference path is identical --
only the default weights dir and the VRAM story differ.

Key facts baked in (this machine has a single ~48 GB GPU):
  * Pick precision with --precision; the 27B weights cost roughly:
        4bit (nf4) ~14 GB ->  ~18 GB peak    (lowest quality)
        8bit (int8)~27 GB ->  ~30 GB peak    (DEFAULT: best quality that fits on 48 GB)
        bf16       ~54 GB -> does NOT fit on one 48 GB GPU (needs multi-GPU / offload)
    4-bit barely touches the card (~18 GB observed), so default to 8-bit for quality.
  ...
  * Linear-attention layers use a slow PyTorch fallback unless flash-linear-attention
    + causal-conv1d are installed (optional; see scripts/qwen3_6_camera.sh header).
...
Run:  bash scripts/qwen3_6_camera.sh --prompt "What is the person doing?"
"""
...
def main() -> None:
    ...
    ap.add_argument("--model", default="model/qwen3_6_27B", help="path to weights dir")
    ...
    ap.add_argument("--precision", choices=["4bit", "8bit", "bf16"], default="8bit",
                    help="weight precision: 4bit nf4 (~18 GB), 8bit int8 (~30 GB, "
                         "default, best quality that fits 48 GB), or full bf16 "
                         "(~54 GB, needs multi-GPU / CPU offload)")
    ...
    print(f"[load] {args.model} ({args.precision}) ...")
    t0 = time.time()
    ...
    model = Qwen3_5ForConditionalGeneration.from_pretrained(args.model, **load_kw)
    model.eval()
    ...
    writer = (ResultWriter(args.results_dir, f"qwen3_6_{_source_tag(args.source)}")
    ...
    win = "Qwen3.6 live  (q quit | SPACE ask | n new q | p pause | t think)"
